...
module.exports = (_ => {
    ...
    if (!global.ZeresPluginLibrary) {
        ...
        return (([Plugin, Api]) => {
            const { WebpackModules, Patcher, ContextMenu, } = Api;
            return class ToneIndicators extends Plugin {
                ...
                getTones(e) { return (e.match(/(^\/+|(?<=\s)\/)\w{0,4}/g) || []).reverse().map(v => [v, tonesList[v]]).filter(v => v[1]) }
                patchMessages() {
                ...
                patchContextMenus() {
                    ContextMenu.getDiscordMenu("MessageContextMenu").then(menu => {
                        Patcher.after(menu, "default", (_, [props], ret) => {
                            ...
                            if (!(selectedText && selectedText.toLowerCase() in tonesList)) return;
                            ...
                            ret.props.children.push(ContextMenu.buildMenuItem({
                                ...
                                action: () => { BdApi.showToast(selectedText + " -  " + tonesList[selectedText.toLowerCase()][0]); }
                            }))
                        })
                    })
                }
